  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-8 px-4 py-10 sm:px-6 sm:py-14">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-[1.5rem] font-bold tracking-[-0.015em] text-slate-900 sm:text-3xl">
            Cześć, {data.student.full_name.split(" ")[0]}
          </h1>
          <p className="text-sm text-muted">{data.student.email}</p>
        </div>
        <Button
          type="button"
          variant="secondary"
        ...
        {data.nextLesson ? (
          <div className="mt-2 space-y-1">
            ...
            {data.nextLesson.location ? (
              <p className="text-sm text-slate-700">{data.nextLesson.location}</p>
            ) : null}
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-slate-900">Twoje materiały</h2>
        {data.materials.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-200 px-4 py-5 text-sm text-muted">
            Na razie pusto. Jak nauczyciel doda linki (PDF, nagrania, tablatury)
            — pojawią się tutaj.
          </p>
        ) : (
      ...
      <section className="space-y-3">
        ...
        {data.pastLessons.length === 0 ? (
          <p className="text-sm text-muted">Jeszcze bez zakończonych terminów.</p>
        ) : (
      ...
      <p className="text-center text-sm text-muted">
        Pytanie?{" "}
        <Link href="/kontakt" className="font-medium text-sky-600 hover:underline">
